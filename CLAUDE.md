...
**Любой новый ролик начинается с опроса владельца** — этапы и стоп-точки в `docs/agent-contract/WORKFLOW.md` (навык `reel-workflow`): модель и усилие → опрос по одному вопросу → текст с ресёрчем и тремя скептиками → монтаж → кадры → рендер.
