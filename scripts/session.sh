...
# при потолке 150k та же сессия стоила бы на 42% меньше. Усилие: режиссёр high, остальные medium.
set -euo pipefail
...
[[ "${1:-}" == "--" ]] && shift
case "$mode" in
  director) effort=high;   cap=150000
  ...
  build)    effort=medium; cap=120000
  ...
  review)   effort=medium; cap=100000
  ...
  research) effort=medium; cap=150000
