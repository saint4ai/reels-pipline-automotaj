...
- Treat face/head clearance as a hard pre-render gate, not a visual guess from one frame. Follow
  `references/FACE_CAPTION_QA.md`: transform detections through the actual crop, keep captions
  below the chin with a safe gap, and switch layout when platform UI leaves no legal slot.
