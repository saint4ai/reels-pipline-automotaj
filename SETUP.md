...
Никогда не сохраняйте ключ в Git, даже в приватном репозитории: обычный коммит поверх
не стирает историю. Используйте переменную окружения или менеджер секретов ОС.
Если раньше вы подключали этот сервер под именем `magic`, уберите старую запись:
