...
## 9. Когда всё готово — сразу опрос

Установка закончена — агент не ждёт команды, а сам переходит к `docs/agent-contract/WORKFLOW.md`:
проверяет модель и усилие (этап 0) и начинает опрос владельца по одному вопросу (этап 1). Монтаж — только после опроса.
