...
- **Текст вылезал за капсулу субтитров на смене строки**: капсула росла 0,22 с, а текст новой строки появлялся сразу. Субтитры — только `studio/src/montage/Captions.tsx` (`capsuleAt`: капсула не уже видимого текста и растёт раньше текста; в qa-режиме — проверка всех стыков строк с шагом 1/120 с). Любой контейнер, меняющий размер, проверять на всём отрезке анимации, а не выборкой `qa-fit` раз в 0,5 с.
