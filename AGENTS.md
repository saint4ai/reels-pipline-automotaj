...
0. **Первое действие после установки и в любой сессии без конкретной задачи — опрос владельца.** Новый ролик начинается с него — `docs/agent-contract/WORKFLOW.md`: модель `gpt-5.6-sol` с `model_reasoning_effort = "xhigh"` (не ниже `"high"`) → опрос по одному вопросу → текст с ресёрчем и тремя скептиками → монтаж → кадры → рендер.
